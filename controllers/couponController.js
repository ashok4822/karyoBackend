...
export const addCoupon = async (req, res) => {
  try {
    ...
    if (!code) return res.status(statusCodes.BAD_REQUEST).json({ message: "Coupon code is required" });
    if (!discountType || !["percentage", "fixed"].includes(discountType)) {
      return res.status(statusCodes.BAD_REQUEST).json({ message: "Discount type must be 'percentage' or 'fixed'" });
    }
    if (!discountValue || discountValue <= 0) {
      return res.status(statusCodes.BAD_REQUEST).json({ message: "Discount value must be greater than 0" });
    }
    if (discountType === "percentage" && discountValue > 100) {
      return res.status(statusCodes.BAD_REQUEST).json({ message: "Percentage discount cannot exceed 100%" });
    }
    if (!validFrom || !validTo) {
      return res.status(statusCodes.BAD_REQUEST).json({ message: "Valid from and valid to dates are required" });
    }
    if (new Date(validFrom) >= new Date(validTo)) {
      return res.status(statusCodes.BAD_REQUEST).json({ message: "Valid to date must be after valid from date" });
    }
    if (
      ...
      return res.status(statusCodes.BAD_REQUEST).json({ message: "Discount value cannot be greater than minimum amount" });
    }
    ...
    if (discountType === "fixed" && maximumDiscount !== undefined && maximumDiscount !== null && maximumDiscount !== "") {
      if (parseFloat(maximumDiscount) < parseFloat(discountValue)) {
        return res.status(statusCodes.BAD_REQUEST).json({ message: "Maximum discount cannot be less than discount value for fixed coupons" });
      }
    }
    if (discountType === "percentage" && maximumDiscount !== undefined && maximumDiscount !== null && maximumDiscount !== "") {
      if (parseFloat(maximumDiscount) <= 0) {
        return res.status(statusCodes.BAD_REQUEST).json({ message: "Maximum discount must be greater than 0 for percentage coupons" });
      }
      if (minimumAmount !== undefined && minimumAmount !== null && parseFloat(minimumAmount) > 0 && parseFloat(maximumDiscount) > parseFloat(minimumAmount)) {
        return res.status(statusCodes.BAD_REQUEST).json({ message: "Maximum discount cannot be greater than minimum amount for percentage coupons" });
      }
    }
    // Check for duplicate code
    const existing = await Coupon.findOne({ code: code.trim().toUpperCase(), isDeleted: false });
    if (existing) {
      return res.status(statusCodes.BAD_REQUEST).json({ message: "Coupon code already exists. Please use a unique code." });
    }
    ...
    await coupon.save();
    res.status(statusCodes.CREATED).json({ message: "Coupon created successfully", coupon });
  } catch (error) {
    res.status(statusCodes.INTERNAL_SERVER_ERROR).json({ message: `Internal Server Error: ${error.message}` });
  }
};
...
export const restoreCoupon = async (req, res) => {
  try {
    ...
    if (!coupon) return res.status(statusCodes.NOT_FOUND).json({ message: "Deleted coupon not found" });
